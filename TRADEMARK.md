# Name and branding policy

The [Elastic License 2.0](LICENSE) covers the source code of this project. It
does **not** grant any rights to the project's name or branding.

## What is not licensed

- The name **Amazing Claude Code GUI** (and the earlier name **Amazing Claude
  Code**), and names confusingly similar to either.
- The plugin logo and the tool window icon, and marks confusingly similar to
  them.
- The plugin ID `io.github.crmapache.amazingclaudecode`, and IDs derived from
  it.

These remain the property of the author, Maksim Zolotoi.

## What you may do

- Use the software, read the source, and modify it, under the terms of the
  Elastic License 2.0.
- Refer to the project by name to state a plain fact - "a fork of Amazing
  Claude Code GUI", "compatible with Amazing Claude Code GUI", "a patch for
  Amazing Claude Code GUI". Descriptive references like these are fine.
- Submit changes back to this repository.

## What you may not do

- Publish a fork or a derivative work under the name Amazing Claude Code GUI, under
  a confusingly similar name, or under the same plugin ID. Pick your own name,
  your own ID, and your own logo.
- Use the name or logo in a way that suggests your fork is this project, or
  that the author endorses, sponsors, or maintains it.
- Remove, alter, or obscure the copyright, license, or authorship notices in
  the software, or the attribution shown in the plugin's own interface. The
  Elastic License 2.0 forbids this outright, regardless of trademark law.

If you distribute a fork, say clearly and near the top of your description that
it is an independent modified version and not the original.

## Third-party marks

Claude and Claude Code are trademarks of Anthropic. This project is an
independent plugin that drives the Claude Code CLI; it is not affiliated with,
endorsed by, or sponsored by Anthropic. JetBrains, IntelliJ, and WebStorm are
trademarks of JetBrains s.r.o., and this project is likewise not affiliated
with JetBrains.

## Questions

If you want to use the name or logo in a way this policy doesn't allow, ask
first: mzpizote@gmail.com.
