...
export interface TaskItem {
   ...
   * его, а итог приходит потом отдельным уведомлением. Поэтому такая карточка
   * переживает конец хода, в отличие от обычной (см. closeUnfinished).
   */
  background?: boolean
}
...
export interface TodoEntry {
   ...
   * именно его, пока пункт в работе; панель — строкой состояния под лентой (см.
   * activityFor в feed/activity.ts). Необязателен: модель вправе его не писать,
   * и тогда происходящее приходится называть по самому пункту.
   */
  activeForm?: string
}
