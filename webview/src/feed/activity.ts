...
export const activityFor = (items: FeedItem[]): string => {
  ...
  if (last?.kind === 'toolGroup' && last.pending) {
    // Идут прямо сейчас, а не вся карточка целиком: в одну группу попадают и
    // вызовы, сделанные один за другим, — назвать их числом значило бы объявить
    // «Running 5 tools» там, где на деле выполняется один.
