...
export const pt: Dict = {
  ...
  menu: {
    titles: {
      ...
      customModels: { title: 'MODELOS PRÓPRIOS', hint: 'os que o Claude Code não oferece' },
    ...
    author: {
      ...
      body: 'Passe nela com um assistente de IA. Teste de graça',
  ...
  newChat: {
    rows: {
      model: { label: 'Modelo', sub: 'Em qual modelo uma aba nova começa' },
      effort: { label: 'Esforço', sub: 'Quanto uma aba nova pensa' },
      mode: { label: 'Modo', sub: 'Com o que as novas abas começam' },
    },
    lastUsed: 'Como da última vez',
    lastUsedNow: (value: string): string => `Segue a última escolha - ${value}`,
  },

  customModels: {
    note: 'O Claude Code nomeia os modelos que conhece, e o menu mostra exatamente esses. Se você trabalha através de um proxy ou de um gateway, escreva aqui os identificadores que ele serve - eles vão ficar no menu ao lado dos demais.',
    warn: 'Num login comum do Claude não há o que adicionar: um nome que o Claude Code não conhece é recusado antes do primeiro turno.',
  ...
  indicators: {
    ...
    modelWeek: {
      label: 'Limite semanal de um modelo',
      named: (model) => `Limite semanal do ${model}`,
      hint: 'A semana própria de um modelo, como na página da conta - só enquanto o plano tiver uma',
    },
  ...
  scenarios: {
    title: 'Cenários',
    hint: 'uma rodada de trabalho escrita uma única vez',
    button: 'Cenários',
    bands: {
      scenarios: 'Cenários',
      runs: 'Execuções',
    ...
    stage: 'Etapa',
    asks: {
      one: 'Uma execução parou e está perguntando algo',
      many: (n: number): string => `${n} execuções pararam e estão perguntando algo`,
      answer: 'Responder',
    },
    draft: {
      title: 'Novo cenário',
      ...
      reading: 'lendo o projeto',
    },
    play: 'Executar',
    unsaved: 'escrito pelo Claude · ainda não salvo',
    unsavedNote: 'Leia antes de guardar: você pediu em uma frase e voltaram etapas de instruções para agentes.',
    discard: 'Descartar',
    when: {
      change: 'Alterar esta execução agendada',
      changeShort: 'Mudar',
      clear: 'Excluir esta execução agendada',
      at: 'Às',
      hours: 'Hora',
      minutes: 'Minutos',
      repeat: 'Repetir',
      ...
      weeklyOn: (day: string): string => `Toda ${day}`,
      next: (when: string): string => `próxima ${when}`,
      then: (when: string): string => `depois ${when}`,
      missed: (when: string): string => `perdida ${when}`,
      missedShort: 'perdida',
      inTime: (reading: string): string => `em ${reading}`,
      today: 'HOJE',
      tomorrow: 'AMANHÃ',
      todayAt: (clock: string): string => `hoje ${clock}`,
      yesterdayAt: (clock: string): string => `ontem ${clock}`,
      noAnswers: 'não pergunta nada',
      another: 'Agendar uma execução',
      newTitle: 'adicionando uma',
      editTitle: 'mudando uma',
      clearTitle: 'Excluir esta execução agendada?',
      orphan: 'O cenário dela não está em nenhuma prateleira',
      scheduled: (count: number): string => `${count} agendadas`,
      /** The file of hours could not be read at all, which is not the same as having none. */
      unread: 'Não foi possível ler as execuções agendadas - o arquivo nesta máquina pode estar danificado. Nada se perdeu: nunca se escreve sobre uma lista que não pôde ser lida.',
      needsIde: 'Só inicia enquanto esta IDE estiver aberta. Um horário que passa com ela fechada não é executado depois: a linha dirá que foi perdido.',
      needsIdeShort: 'só começa com esta IDE aberta',
      runNow: 'Executar agora',
      itAsksFor: 'ELE PERGUNTA',
      answeredNow: 'Respondido agora, porque naquela hora não há ninguém no teclado.',
    ...
    queue: {
      ...
      waitingHere: (n) => (n === 1 ? '1 execução esperando na fila' : `${n} execuções esperando na fila`),
      placeFirst: 'Vai ser a primeira da fila.',
      placeBehind: (name) => `Vai ser a primeira da fila e começa quando ${name} terminar.`,
      place: (ahead) => (ahead === 1 ? 'Há 1 execução na frente dela.' : `Há ${ahead} execuções na frente dela.`),
      startWhen: 'QUANDO COMEÇAR',
      afterSuccess: 'Depois que a anterior terminar bem',
      afterSuccessNote: 'Se aquela falhar ou for parada, a fila para aqui e espera por você.',
      afterAnything: 'Depois da anterior, aconteça o que acontecer',
      afterAnythingNote: 'Começa mesmo se a anterior falhar. Para trabalho que não depende dela.',
      waitsForSuccess: 'depois de uma boa',
      waitsForAnything: 'depois de qualquer uma',
      switchToAny: 'Começar mesmo se a anterior falhar',
      switchToSuccess: 'Começar só depois de uma boa',
      wouldNotStart: (why) => `não começou: ${why}`,
      moveUp: 'Subir na fila',
      moveDown: 'Descer na fila',
      remove: 'Tirar da fila',
      stopped: 'A fila parou',
      stoppedOn: (name, why) => `${name} terminou: ${why}. Nada depois disso vai começar.`,
      stoppedUnknown: 'não dá para saber como terminou',
      goOn: 'Começar a próxima',
      goOnBehind: 'Seguir depois da atual',
      goOnBehindHint: (name) => `A próxima começa quando ${name} terminar.`,
      clear: 'Esvaziar a fila',
      clearTitle: 'Esvaziar a fila?',
      clearSubject: (n) => (n === 1 ? '1 execução esperando' : `${n} execuções esperando`),
      goingNow: 'Rodando agora',
      empty: 'Não há nada na fila',
    ...
    needsFixing: 'precisa de ajustes antes de poder rodar',
    ...
    besideGoing: (n: number): string =>
      n === 1
        ...
        : `Vai começar uma execução ao lado das ${n} que já correm. Todas trabalham na mesma cópia de trabalho.`,
    pastRuns: 'EXECUÇÕES ANTERIORES',
    newestFirst: 'mais recentes primeiro',
    noRuns: 'Nada foi executado ainda.',
    nothingRunning: 'Agora não há nada em andamento neste projeto.',
    spentToday: (amount: string): string => `${amount} hoje`,
    runsKept: (n: number): string => (n === 1 ? '1 execução guardada' : `${n} execuções guardadas`),
    table: {
      run: 'EXECUÇÃO',
      started: 'COMEÇOU',
      cards: 'CARTÕES',
    ...
    shelves: {
      ...
      user: 'EM TODOS OS PROJETOS',
      userNote: 'só seu · acompanha você de projeto em projeto',
      projectEmpty: 'Ainda não há cenário compartilhado',
      projectEmptyNote: 'Ponha um aqui e ele fica ao lado dos comandos e skills do projeto.',
      noProject: 'Esta janela não tem pasta de projeto',
      noProjectNote: 'Não há onde pôr um cenário que o repositório levaria junto.',
      userEmpty: 'Ainda não há nada seu',
      userEmptyNote: 'Um cenário posto aqui é só seu e acompanha você de projeto em projeto.',
    },
    runStates: {
      starting: 'Iniciando',
      running: 'Em andamento',
      paused: 'Em pausa',
      blocked: 'Esperando por você',
      done: 'Concluída',
      failed: 'Falhou',
      stopped: 'Interrompida por você',
    },
    stepStates: {
      waiting: 'Ainda não',
      running: 'Trabalhando',
      asking: 'Perguntando',
      judging: 'Com a thread principal',
      paused: 'Em pausa',
      done: 'Pronto',
      failed: 'Falhou',
      skipped: 'Nunca rodou',
    ...
    failures: {
      noHead: 'A thread principal nunca subiu',
      noStart: 'Não chegou a iniciar',
      tooLong: 'Passou do tempo',
      headSilent: 'A thread principal parou de responder',
      crashed: 'O processo sumiu',
      refused: 'Recusado',
    ...
    outcomes: {
      ...
      scenarioNotWritten: 'Não foi possível gravar o cenário no disco.',
      scenarioNotDeleted: 'Não foi possível excluir o cenário.',
      schedulesNotWritten: 'Não foi possível gravar as execuções agendadas no disco.',
      scenarioGone: 'Esse cenário não existe mais.',
      runBusy: 'Essa execução ainda está em andamento - pare-a primeiro.',
    ...
    run: {
      ...
      stop: 'Parar',
      open: 'Abrir',
      stopTitle: 'Parar esta execução?',
      stopSubject: 'O que um cartão estiver fazendo agora é cortado, e as etapas seguintes não chegam a acontecer.',
      after: {
        stopped: (card: string): string =>
          card ? `Você a interrompeu em “${card}”. Nada depois desse cartão chegou a começar.` : 'Você a interrompeu antes de o primeiro cartão começar.',
        failed: (card: string): string =>
          card ? `Parou por uma falha em “${card}”.` : 'Parou por uma falha antes de o primeiro cartão começar.',
        done: 'Todos os cartões estão feitos.',
        carryOn: 'Continuar a execução',
        carryOnHint: 'Retoma de onde parou: a mesma thread principal com tudo o que lembra, e o cartão cortado é mandado seguir.',
        chat: 'Continuar em um chat',
        chatHint: 'Abre a conversa da thread principal em uma aba própria. Ela lembra toda a execução, então dá para seguir dali.',
      },
      cards: (done, total) => `${done}/${total} cartões`,
      running: 'rodando há',
      runningShort: 'há',
      openFor: 'aberta há',
      took: 'durou',
      ...
      passOf: (pass, passes) => `passada ${pass} de ${passes}`,
      ...
      deny: 'Recusar',
      send: 'Enviar',
      answerPlaceholder: 'O que responder a ele…',
      asking: 'ELE ESTÁ PERGUNTANDO',
      nothingSpent: 'enquanto isto está de pé, nada é gasto',
      stageDone: (took: string): string => `pronta · ${took}`,
      stageHere: 'aqui agora',
    ...
    help: {
      ...
      title: 'Cenários, em resumo',
      what: { lead: 'Um cenário', text: 'é uma rodada de trabalho que você repete, escrita uma única vez: alguns cartões, cada um dito a uma sessão do Claude só dele, um depois do outro.' },
      asks: { lead: 'Ele pode perguntar algo antes de começar', text: 'um número de ticket, um branch, o que mudar de uma execução para a outra - assim a mesma rodada de trabalho vai contra outra coisa a cada vez.' },
      run: { lead: 'Aperte Executar', text: 'e a thread principal percorre tudo por você: conta a cada cartão o que os anteriores descobriram, decide se está pronto e responde às perguntas em que eles param.' },
      kept: { lead: 'Guardado no repositório', text: 'o cenário viaja junto e fica com todo mundo que trabalha ali; guardado em todos os projetos é só seu e acompanha você.' },
      watch: { lead: 'Uma execução abre numa aba própria', text: 'com cada passo em ordem. Pause, continue ou pare - e aperte qualquer passo para ler a conversa inteira dele.' },
    },
    log: {
      loading: 'Lendo o que ele disse…',
      step: 'Passo',
      head: 'Thread principal',
    ...
    editor: {
      ...
      save: 'Salvar',
      problems: 'Não dá para executar enquanto isto não for corrigido:',
      warnings: 'VALE ARRUMAR',
      issues: (n: number): string => (n === 1 ? '1 problema' : `${n} problemas`),
      outline: 'O CENÁRIO',
      nameAndShelf: 'Nome e prateleira',
      name: 'NOME',
      shelf: 'GUARDADO EM',
      inRepository: 'Este repositório',
      mine: 'Todos os projetos',
      head: 'Thread principal',
      headNote: 'lê isto e julga cada cartão por isso',
      briefingHint: 'Para que serve esta rodada de trabalho, nas suas palavras. A thread principal lê isto e julga cada cartão por isso.',
      defaultModel: 'Como uma aba nova',
      defaultEffort: 'Como uma aba nova',
      sameAsHead: 'Igual à thread principal',
      ...
      inputs: 'Pergunta',
      inputsLabel: 'PERGUNTADO ANTES DE COMEÇAR',
      addInput: 'Adicionar uma pergunta',
      noInputs: 'Este cenário não pergunta nada antes de começar.',
      inputName: 'nome',
      inputLabel: 'Como chamar no formulário',
      required: 'obrigatório',
      stages: 'ETAPAS',
      addStage: 'Adicionar uma etapa',
      stageNumber: (n) => `Etapa ${n}`,
      stageHead: (n: number, title: string): string => `ETAPA ${n} · ${title}`,
      passes: 'PASSADAS',
      once: 'uma',
      times: (n) => `${n}`,
      goesRound: (n: number): string => `dá ×${n} voltas`,
      goesRoundUntil: (n: number): string => `dá ×${n} voltas, até estar pronto`,
      roundsShort: (n: number): string => `×${n}`,
      roundsUntilShort: (n: number): string => `×${n}, até pronto`,
      untilDone: 'ou menos, se a thread principal disser que está pronto',
      cards: 'CARTÕES',
      addCard: 'Adicionar um cartão',
      addCardHere: 'Adicionar um cartão a esta etapa',
      cardTitle: 'Como se chama este cartão',
      prompt: 'O QUE É DITO À SESSÃO DELE',
      promptHint: 'O que será dito à sessão própria deste cartão. {{nome}} é perguntado antes da execução; [[nome]] a thread principal preenche com o que os cartões acima descobriram.',
      promptNote: 'Os nomes de espaço vêm destacados. É o único campo na fonte de console.',
      slots: 'ESPAÇOS QUE ELE PREENCHE',
      noSlots: 'Nenhum. Escreva [[nome]] no texto e descreva aqui.',
      slotName: 'nome',
      slotHint: 'O que a thread principal deve colocar aqui',
      addSlot: 'Adicionar um espaço',
      dod: 'PRONTO QUANDO',
      dodHint: 'Como a thread principal vai saber que este cartão terminou. Vazio significa que basta o turno acabar.',
      after: 'PASSAR ADIANTE',
      afterHint: 'O que a thread principal deve fazer quando o cartão estiver pronto: conferir algo, anotar algo.',
      overrides: 'RODA COM',
      moveUp: 'Subir',
      moveDown: 'Descer',
      remove: 'Remover',
      untitledCard: 'Um cartão sem nome',
      untitledStage: 'Uma etapa sem nome',
    },
    problems: {
      noStages: 'Não há nenhuma etapa nele.',
      emptyStage: 'Uma etapa não tem nenhum cartão.',
      noPrompt: (card) => `${card}: não há nada escrito para a sessão dele fazer.`,
      unknownInput: (card, name) => `${card}: {{${name}}} não é algo que este cenário pergunte.`,
      undeclaredSlot: (card, name) => `${card}: [[${name}]] está no texto, mas ninguém é encarregado de preencher.`,
      unusedSlot: (card, name) => `${card}: o espaço ${name} está descrito mas não é usado no texto.`,
      duplicateInput: (name) => `Duas perguntas têm o mesmo nome: ${name}.`,
      duplicateSlot: (card, name) => `${card}: dois espaços têm o mesmo nome: ${name}.`,
      badInputName: (name) => `"${name}" não é um nome - use letras, dígitos, - e _.`,
      badSlotName: (card, name) => `${card}: "${name}" não é um nome - use letras, dígitos, - e _.`,
    },
  },
  ...
  models: {
    ...
    fable: { sub: 'Fable 5.1 · Para as tarefas mais difíceis e mais longas' },
    ...
    add: 'Adicionar um modelo…',
    addSub: 'Um que o Claude Code não oferece',
  },
  ...
  header: {
    ...
    scenarios: 'Cenários',
    closeScenarios: 'Fechar os cenários',
  ...
  feed: {
    ...
    workflow: {
      ...
      cached: 'do registro',
      steps: 'O QUE ELE FEZ',
      returned: 'O QUE ELE DEVOLVEU',
      cut: 'Cortado - a resposta continua além do que cabe aqui.',
      noTranscript: 'O registro dele não está mais no disco - isto é o resumo do relatório.',
      reading: 'Lendo o registro dele…',
    },
    ...
    tool: {
      ...
      closed: {
        ...
        restarted: 'O Claude Code reiniciou aqui e isto não sobreviveu.',
      },
      closedMeta: {
    ...
    task: {
      ...
      closed: {
        ...
        restarted: 'O Claude Code reiniciou aqui e este agente não sobreviveu.',
      },
    },
    ...
    result: {
      ...
      restarted: 'O Claude Code reiniciou - tudo o que ainda rodava foi interrompido',
    },
  ...
  mobile: {
    ...
    sessions: {
      ...
      state: {
        crashed: 'parada - o processo morreu',
        waiting: 'espera por você',
        waitingPermission: 'espera por você · permissão',
        waitingQuestion: 'espera por você · pergunta',
        waitingPlan: 'espera por você · plano',
        working: 'trabalhando',
        done: 'pronta',
      },
      reach: {
    ...
    thread: {
      ...
      stopAgent: (what) => `Parar ${what}?`,
    },

    drawer: {
      menu: 'Menu',
      projects: 'Projetos',
      tasks: 'Tarefas e agentes',
      mcp: 'Servidores MCP',
      plugins: 'Plugins',
      ...
      live: (n) => `${n} ativos`,
      sealed: 'Selado entre a sua IDE e este telefone.',
    },
    ...
    scenarios: {
      ...
      row: {
        runNow: 'Executar agora',
        asksFirst: (names: string): string => `pergunta ${names}`,
        schedule: 'Agendar uma execução',
        scheduled: (n: number): string => `${n} marcadas`,
        editor: 'Abrir o editor',
        duplicate: 'Duplicar',
        delete: 'Apagar este cenário',
      },
      start: {
      ...
      editor: {
        tidy: (n: number): string => (n === 1 ? '1 coisa a arrumar' : `${n} coisas a arrumar`),
        tooBig: 'Este cenário é grande demais para abrir daqui. Ele pode ser editado na mesa.',
        card: (stage: number, at: number, total: number): string => `etapa ${stage} · cartão ${at} de ${total}`,
        done: 'Pronto',
      },
      step: {
        told: 'O QUE FOI DITO A ELE',
        verdict: 'VEREDITO',
        of: (at: number, took: string): string => `passo ${at} · ${took}`,
      },
    },
    ...
    tasks: {
      title: 'Tarefas e agentes',
      label: 'Lista de tarefas',
      doneOf: (done, total) => `${done} de ${total} prontas`,
      running: 'EM CURSO',
      agents: 'Agentes',
      agent: 'Subagente',
      background: 'Em segundo plano',
      nothing: 'Nada está rodando nesta conversa.',
      agentGone: 'Este agente não está mais na conversa em tela.',
    },

    run: {
      title: 'Como este turno roda',
      subtitle: 'vale só para esta conversa',
      apply: 'Aplicar',
      locked: 'travado',
      inForce: 'em vigor agora',
      modeNote: 'Esta conversa começou na mesa e pode haver alguém sentado à frente dela, então o modo de permissões não é alterado daqui. Uma conversa iniciada pelo telefone começa no modo que você escolher.',
    },

    message: {
      title: 'Esta mensagem',
      quote: 'Citar na minha próxima mensagem',
      fork: 'Bifurcar daqui',
      forkHint: 'Uma conversa nova com tudo até este ponto.',
      copy: 'Copiar',
      pin: 'Fixar acima da conversa',
      unpin: 'Desafixar',
      pinsFull: 'Já há três fixadas - desafixe uma primeiro.',
    },
    ...
    plugins: {
      ...
      readOnly: 'Instalar um plugin e desligar um continuam na mesa: eles rodam código de terceiros naquela máquina. O que serve aqui é ver quais skills e comandos uma conversa realmente tem.',
      noneInstalled: 'Nenhum plugin instalado.',
      nothingFound: 'Nada corresponde.',
      noMarkets: 'Nenhuma fonte conectada.',
      search: 'Buscar no catálogo',
      on: 'ligado',
      off: 'desligado',
      trimmed: 'O catálogo é cortado ao que cabe numa mensagem - o resto está na mesa.',
    },

    accounts: {
      subtitle: 'qual assinatura paga o trabalho',
      paying: 'Paga por esta conversa',
      none: 'Apenas o login que o Claude Code já tinha.',
      switchAsk: (name) => `Trabalhar em ${name} de agora em diante? Todas as conversas abertas naquela máquina passam para ela, e um turno em curso é interrompido.`,
      switchNote: 'Trocar de conta move todas as conversas abertas daquela máquina para a nova e interrompe qualquer turno em curso.',
      addNote: 'Adicionar uma conta e autorizar o Claude Design continuam na mesa: os dois abrem um terminal e um login de navegador naquela máquina.',
      forgetAsk: (name) => `Esquecer ${name}? A credencial sai daquela máquina; a conta em si fica intacta e entrar de novo a traz de volta.`,
      logoutAsk: 'Sair do Claude Code? Isso revoga a credencial na Anthropic - você sai em todas as suas máquinas.',
    },
  ...
  status: {
    ...
    modelWeekLimit: (model) => `Limite semanal do ${model}`,
