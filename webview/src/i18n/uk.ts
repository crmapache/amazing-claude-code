...
export const uk: Dict = {
  ...
  settings: {
    rows: {
      ...
      calmColors: { label: 'Спокійні кольори', sub: 'Шкали одним рівним тоном' },
  ...
  calmColors: {
    ...
    label: 'Спокійні кольори',
    hint: 'Один тон, хоч би що шкали показували',
  ...
  scenarios: {
    ...
    draft: {
      ...
      keys: 'Спершу читається проєкт, тож це займе близько півхвилини.',
    ...
    lastUsed: (value: string): string => `Минулого разу було ${value}`,
    besideGoing: (n) => {
      const rest = n % 100
      const last = n % 10
    ...
    log: {
      ...
      truncated: 'Це кінець - початок не показано.',
    },
    editor: {
      title: 'СЦЕНАРІЙ',
