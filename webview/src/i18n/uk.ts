...
export const uk: Dict = {
  ...
  scenarios: {
    ...
    button: 'Сценарії',
    untested: {
      text: 'Сценарії тут нові й ще не перевірені з усіх боків. Якщо щось піде не так, опишіть це у формі зворотного зв\'язку - лише так це й лагодиться.',
    ...
    draft: {
      ...
      hint: 'Опишіть коло роботи, і Клод запише його: що зробити, у якому порядку і що має бути правдою наприкінці.',
    ...
    play: 'Запустити',
    when: {
      set: 'Запускати за часом',
      change: 'Змінити час',
      clear: 'Більше не запускати за часом',
      at: 'О',
      hours: 'Година',
      minutes: 'Хвилини',
      repeat: 'Повтор',
      ...
      ran: (when: string): string => `запускався ${when}`,
    ...
    busy: 'У цьому проєкті вже йде запуск',
    ...
    shelves: {
      ...
      user: 'У ВСІХ ПРОЄКТАХ',
      projectEmpty: 'Поки порожньо. Сценарій звідси їде разом із репозиторієм, поруч із командами та скілами проєкту.',
      noProject: 'У цього вікна немає теки проєкту, тож спільний сценарій нікуди покласти.',
      userEmpty: 'Поки порожньо. Сценарій звідси лише ваш і ходить із вами від проєкту до проєкту.',
    },
    runStates: {
    ...
    outcomes: {
      ...
      scenarioBusy: 'У цьому проєкті вже йде запуск. По одному за раз - інакше двоє правлять ті самі файли.',
    ...
    editor: {
      ...
      warnings: 'Варто причепурити:',
      name: 'НАЗВА',
      shelf: 'ЛЕЖИТЬ У',
      inRepository: 'У цьому репозиторії',
      mine: 'У всіх проєктах',
      head: 'ГОЛОВНИЙ ПОТІК',
      ...
      questionHead: 'Відповідає головний потік',
      questionStop: 'Стати й чекати на мене',
      retries: 'ПОВЕРНЕННЯ',
      noRetries: 'Одна спроба, без других шансів',
      retriesCount: (n) => {
        const rest = n % 100
        const last = n % 10
        if (last === 1 && !(rest >= 11 && rest <= 14)) return 'не більше ніж 1 раз'
        return `не більше ніж ${n} раз`
      },
      inputs: 'ЗАПИТУЄТЬСЯ ПЕРЕД ЗАПУСКОМ',
      ...
      overrides: 'ЛИШЕ ДЛЯ ЦІЄЇ КАРТКИ',
      moveUp: 'Вище',
      moveDown: 'Нижче',
      remove: 'Прибрати',
  ...
  mobile: {
    ...
    scenarios: {
      running: 'ЗАРАЗ ІДЕ',
      nothingRunning: 'Зараз у цьому проєкті нічого не виконується.',
      none: 'У цьому проєкті не записано жодного кола робіт.',
      deskNote: 'Сценарії пишуть, запускають і ставлять на час за столом. Звідси можна стежити за прогоном, відповідати на його запитання та зупиняти його.',
    },
