...
export const uk: Dict = {
  ...
  menu: {
    titles: {
      ...
      composerLayout: { title: 'РОЗКЛАДКА ПОЛЯ ВВЕДЕННЯ', hint: 'де стоїть поле введення' },
    ...
    rows: {
      ...
      settings: { label: 'Налаштування', sub: 'Звуки, режим, розкладка, мова' },
  ...
  settings: {
    rows: {
      ...
      composerLayout: { label: 'Розкладка поля введення', sub: 'Де стоїть поле введення' },
  ...
  history: {
    empty: 'Минулих розмов тут поки немає.',
    today: 'СЬОГОДНІ',
    earlier: 'РАНІШЕ',
    messages: (n) => {
      const rest = n % 100
  ...
  search: {
    ...
    aiNote: 'Claude прочитає розмови цього проєкту · окремий запуск, іде у вашу витрату',
    find: 'Знайти',
    cancel: 'Скасувати',
    retry: 'Повторити',
    copy: 'Копіювати',
  ...
  voice: {
    ...
    multiHint: 'Мультимова стежить за зміною мови посеред фрази. На вимірах вона програє названій мові в обох випадках - беріть її, тільки якщо ви справді мішаєте дві мови в одному реченні.',
  ...
  feed: {
    ...
    limit: {
      ...
      waiting: (window) => `${window ? `Ліміт «${window}»` : 'Ліміт підписки'} вичерпано - чекаємо, коли він обнулиться`,
  ...
  remote: {
    ...
    states: {
      ...
      refused: {
        label: 'Відмовлено',
        hint: 'Релей не прийняв цей плагін: можливо, він застарий, або ця адреса зайнята іншою IDE.',
      },
    },
    agent: (id) => `агент ${id}`,
    thisIde: 'ЦЯ IDE',
    relay: 'РЕЛЕЙ',
    device: 'ПРИСТРІЙ',
    allow: 'Дозволити діставатися цієї IDE віддалено',
  ...
  feedback: {
    ...
    logsWhat:
      'версії, тайминги і те, що пішло не так. Ні вашої розмови, ні імен файлів, ні шляхів - і все це можна прочитати цілком до відправлення.',
    ...
    reportNote: (tab) =>
      `Це все вкладення цілком, слово в слово${tab ? `, для вкладки ${tab}` : ''}. Воно збирається тут, у вашій IDE, з того, що бачив сам плагін: версії, форма тієї розмови і все, що впало. Імена файлів ідуть короткими хешами, тож той самий файл читається як той самий, але який саме - не говориться.`,
    building: 'Збираємо…',
    copy: 'Скопіювати',
    problems: {
  ...
  mobile: {
    ...
    sessions: {
      nothingYet: 'Поки нічого показати. Відкрийте проєкт в IDE або прив’яжіть ще одну.',
      nonePaired: 'До цього телефона поки не прив’язано жодної IDE. Натисніть «Прив’язати».',
      ...
      reach: {
        ...
        elsewhere: 'Відкрито ще в одній вкладці або у встановленій програмі - з’єднання тримає та копія.',
        reconnecting: 'Перепідключаємося… список нижче може бути застарілим.',
        offline: 'До релея не достукатися. Нічого не втрачено - зв’язок повернеться сам.',
      },
      agent: {
    ...
    history: { title: 'Історія', empty: 'Минулих розмов у цьому проєкті поки немає.' },
    ...
    limits: {
      ...
      resetUnknown: 'час обнулення поки невідомий',
      resetsIn: (left) => `обнулиться через ${left}`,
    },
  },
  ...
  commands: {
    ...
    usage: 'вікна підписки і коли вони обнуляться',
    codeReview: 'прорев’юити pull request',
  },
}
