...
export const de: Dict = {
  ...
  menu: {
    titles: {
      ...
      customModels: { title: 'EIGENE MODELLE', hint: 'die, die Claude Code nicht anbietet' },
    ...
    author: {
      ...
      body: 'Bestehe es mit einem KI-Assistenten. Kostenlos ausprobieren',
  ...
  newChat: {
    rows: {
      model: { label: 'Modell', sub: 'Mit welchem Modell ein neuer Tab startet' },
      effort: { label: 'Aufwand', sub: 'Wie gründlich ein neuer Tab denkt' },
      mode: { label: 'Modus', sub: 'Womit neue Tabs starten' },
    },
    lastUsed: 'Wie zuletzt gewählt',
    lastUsedNow: (value: string): string => `Folgt der letzten Wahl - ${value}`,
  },

  customModels: {
    note: 'Claude Code nennt die Modelle, die es kennt, und das Menü zeigt genau diese. Wer über einen Proxy oder ein Gateway arbeitet, trägt hier die Bezeichner ein, die dort bedient werden - sie stehen dann im Menü neben den übrigen.',
    warn: 'Bei einer gewöhnlichen Claude-Anmeldung gibt es nichts hinzuzufügen: ein Name, den Claude Code nicht kennt, wird schon vor dem ersten Zug abgelehnt.',
    placeholder: 'Modellbezeichner',
    add: 'Hinzufügen',
    remove: 'Entfernen',
    added: 'HINZUGEFÜGT',
    none: 'Keine',
    count: (n) => (n === 1 ? '1 Modell' : `${n} Modelle`),
  },
  ...
  scenarios: {
    title: 'Szenarien',
    hint: 'ein Arbeitsablauf, einmal aufgeschrieben',
    button: 'Szenarien',
    bands: {
      scenarios: 'Szenarien',
      runs: 'Läufe',
    ...
    create: 'Neues Szenario',
    newName: 'Neues Szenario',
    stage: 'Stufe',
    asks: {
      one: 'Ein Lauf ist stehen geblieben und fragt dich etwas',
      many: (n: number): string => `${n} Läufe sind stehen geblieben und fragen dich etwas`,
      answer: 'Beantworten',
    },
    draft: {
      ...
      reading: 'liest das Projekt',
    },
    play: 'Starten',
    unsaved: 'von Claude geschrieben · noch nicht gespeichert',
    unsavedNote: 'Lies es, bevor du es behältst: du hast in einem Satz gefragt und Stufen von Anweisungen an Agenten zurückbekommen.',
    discard: 'Verwerfen',
    when: {
      change: 'Diesen geplanten Lauf ändern',
      changeShort: 'Ändern',
      clear: 'Diesen geplanten Lauf löschen',
      at: 'Um',
      hours: 'Stunde',
      minutes: 'Minuten',
      repeat: 'Wiederholung',
      ...
      weeklyOn: (day: string): string => `Jeden ${day}`,
      next: (when: string): string => `nächster ${when}`,
      then: (when: string): string => `dann ${when}`,
      missed: (when: string): string => `verpasst ${when}`,
      missedShort: 'verpasst',
      inTime: (reading: string): string => `in ${reading}`,
      today: 'HEUTE',
      tomorrow: 'MORGEN',
      todayAt: (clock: string): string => `heute ${clock}`,
      yesterdayAt: (clock: string): string => `gestern ${clock}`,
      noAnswers: 'fragt nichts ab',
      another: 'Lauf einplanen',
      newTitle: 'wird hinzugefügt',
      editTitle: 'wird geändert',
      clearTitle: 'Diesen geplanten Lauf löschen?',
      orphan: 'Sein Szenario liegt in keinem der beiden Regale',
      scheduled: (count: number): string => `${count} geplant`,
      /** The file of hours could not be read at all, which is not the same as having none. */
      unread: 'Die geplanten Läufe konnten nicht gelesen werden - die Datei auf diesem Rechner ist möglicherweise beschädigt. Nichts ist verloren: Über eine Liste, die nicht gelesen werden konnte, wird nie geschrieben.',
      needsIde: 'Startet nur, solange diese IDE offen ist. Eine Uhrzeit, die bei geschlossener IDE vergeht, wird nicht nachgeholt - in der Zeile steht dann, dass sie verpasst wurde.',
      needsIdeShort: 'startet nur, solange diese IDE offen ist',
      runNow: 'Jetzt starten',
      itAsksFor: 'ES FRAGT NACH',
      answeredNow: 'Jetzt beantwortet, denn zu dieser Uhrzeit sitzt niemand an der Tastatur.',
      save: 'Uhrzeit setzen',
    ...
    queue: {
      ...
      waitingHere: (n) => (n === 1 ? '1 Lauf wartet in der Warteschlange' : `${n} Läufe warten in der Warteschlange`),
      placeFirst: 'Er steht als Erstes in der Warteschlange.',
      placeBehind: (name) => `Er steht als Erstes in der Warteschlange und startet, sobald ${name} fertig ist.`,
      place: (ahead) => (ahead === 1 ? '1 Lauf steht davor.' : `${ahead} Läufe stehen davor.`),
      startWhen: 'WANN STARTEN',
      afterSuccess: 'Wenn der davor sauber durchgelaufen ist',
      afterSuccessNote: 'Scheitert der oder wird er gestoppt, bleibt die Warteschlange hier stehen und wartet auf dich.',
      afterAnything: 'Wenn der davor fertig ist, egal wie',
      afterAnythingNote: 'Startet auch, wenn der davor gescheitert ist. Für Arbeit, die nicht davon abhängt.',
      waitsForSuccess: 'nach einem sauberen Lauf',
      waitsForAnything: 'nach jedem Ende',
      switchToAny: 'Auch starten, wenn der davor scheitert',
      switchToSuccess: 'Nur nach einem sauberen Lauf starten',
      wouldNotStart: (why) => `ließ sich nicht starten: ${why}`,
      moveUp: 'In der Warteschlange nach oben',
      moveDown: 'In der Warteschlange nach unten',
      remove: 'Aus der Warteschlange nehmen',
      stopped: 'Die Warteschlange steht',
      stoppedOn: (name, why) => `${name} endete: ${why}. Danach startet nichts mehr.`,
      stoppedUnknown: 'wie er endete, ist nicht bekannt',
      goOn: 'Den nächsten starten',
      goOnBehind: 'Nach dem laufenden weitermachen',
      goOnBehindHint: (name) => `Der nächste startet, sobald ${name} fertig ist.`,
      clear: 'Warteschlange leeren',
      clearTitle: 'Warteschlange leeren?',
      clearSubject: (n) => (n === 1 ? '1 Lauf wartet' : `${n} Läufe warten`),
      goingNow: 'Läuft gerade',
      empty: 'Nichts steht an',
    ...
    needsFixing: 'muss erst in Ordnung gebracht werden',
    problemsCount: (n: number): string => (n === 1 ? '1 Problem' : `${n} Probleme`),
    fix: 'In Ordnung bringen',
    stages: (n) => (n === 1 ? '1 Stufe' : `${n} Stufen`),
    ...
    besideGoing: (n: number): string =>
      n === 1
        ...
        : `Es startet einen Lauf neben den ${n}, die schon laufen. Alle arbeiten auf derselben Arbeitskopie.`,
    pastRuns: 'FRÜHERE LÄUFE',
    newestFirst: 'neueste zuerst',
    noRuns: 'Es wurde noch nichts gestartet.',
    nothingRunning: 'In diesem Projekt läuft gerade nichts.',
    spentToday: (amount: string): string => `${amount} heute`,
    runsKept: (n: number): string => (n === 1 ? '1 Lauf aufbewahrt' : `${n} Läufe aufbewahrt`),
    table: {
      run: 'LAUF',
      started: 'GESTARTET',
      cards: 'KARTEN',
    ...
    shelves: {
      ...
      user: 'IN JEDEM PROJEKT',
      userNote: 'nur deins · begleitet dich von Projekt zu Projekt',
      projectEmpty: 'Noch kein geteiltes Szenario',
      projectEmptyNote: 'Leg eins hierher, dann liegt es neben den Befehlen und Skills des Projekts.',
      noProject: 'Dieses Fenster hat keinen Projektordner',
      noProjectNote: 'Es gibt keinen Platz für ein Szenario, das das Repository mitträgt.',
      userEmpty: 'Noch nichts Eigenes',
      userEmptyNote: 'Ein Szenario an dieser Stelle gehört nur dir und begleitet dich von Projekt zu Projekt.',
    },
    runStates: {
      starting: 'Startet',
      running: 'Läuft',
    ...
    stepStates: {
      ...
      running: 'Arbeitet',
      asking: 'Fragt nach',
      judging: 'Beim Hauptstrang',
      paused: 'Pausiert',
      done: 'Fertig',
      failed: 'Fehlgeschlagen',
      skipped: 'Nie gelaufen',
    },
    failures: {
      noHead: 'Der Hauptstrang kam nie hoch',
      noStart: 'Ließ sich nicht starten',
      tooLong: 'Hat die Zeit überschritten',
      headSilent: 'Der Hauptstrang antwortet nicht mehr',
      crashed: 'Der Prozess ist verschwunden',
      refused: 'Abgelehnt',
      noVerdict: 'Kein Urteil',
    ...
    outcomes: {
      ...
      scenarioNotWritten: 'Das Szenario konnte nicht auf die Platte geschrieben werden.',
      scenarioNotDeleted: 'Das Szenario konnte nicht gelöscht werden.',
      schedulesNotWritten: 'Die geplanten Läufe konnten nicht auf die Festplatte geschrieben werden.',
      scenarioGone: 'Dieses Szenario gibt es nicht mehr.',
      runBusy: 'Dieser Lauf läuft noch - halte ihn zuerst an.',
    ...
    run: {
      title: 'LAUF',
      startedAt: (when) => `gestartet ${when}`,
      head: 'Hauptstrang',
      pause: 'Pause',
      resume: 'Weiter',
      stop: 'Stopp',
      open: 'Öffnen',
      stopTitle: 'Diesen Lauf stoppen?',
      stopSubject: 'Was eine Karte gerade tut, wird abgebrochen, und die Stufen danach finden gar nicht erst statt.',
      after: {
        stopped: (card: string): string =>
          card ? `Du hast ihn bei „${card}“ gestoppt. Nichts nach dieser Karte wurde gestartet.` : 'Du hast ihn gestoppt, bevor die erste Karte begann.',
        failed: (card: string): string =>
          card ? `Er ist bei „${card}“ an einem Fehler stehen geblieben.` : 'Er ist an einem Fehler stehen geblieben, bevor die erste Karte begann.',
        done: 'Alle Karten sind erledigt.',
        carryOn: 'Lauf fortsetzen',
        carryOnHint: 'Macht dort weiter, wo er stand: derselbe Hauptstrang mit allem, was er sich gemerkt hat, und die abgebrochene Karte wird gebeten weiterzumachen.',
        chat: 'Im Chat fortsetzen',
        chatHint: 'Öffnet das Gespräch des Hauptstrangs in einem eigenen Tab. Er erinnert sich an den ganzen Lauf, also kannst du dort weitermachen.',
      },
      cards: (done, total) => `${done}/${total} Karten`,
      running: 'läuft seit',
      runningShort: 'seit',
      openFor: 'offen seit',
      took: 'dauerte',
      cost: 'kostete',
      tokens: 'Tokens',
      stageOf: (at: number, total: number): string => `Stufe ${at} von ${total}`,
      passOf: (pass, passes) => `Durchgang ${pass} von ${passes}`,
      ...
      deny: 'Ablehnen',
      send: 'Senden',
      answerPlaceholder: 'Was ihr geantwortet werden soll…',
      asking: 'ES FRAGT',
      nothingSpent: 'solange das steht, wird nichts verbraucht',
      stageDone: (took: string): string => `fertig · ${took}`,
      stageHere: 'gerade hier',
    ...
    help: {
      ...
      title: 'Szenarien, kurz gesagt',
      what: { lead: 'Ein Szenario', text: 'ist ein Arbeitsablauf, den du wiederholst, einmal aufgeschrieben: ein paar Karten, jede einer eigenen Claude-Sitzung gesagt, eine nach der anderen.' },
      asks: { lead: 'Es kann dich vorher etwas fragen', text: 'eine Ticketnummer, einen Branch, was sich von Lauf zu Lauf ändert - so geht derselbe Arbeitsablauf jedes Mal gegen etwas anderes.' },
      run: { lead: 'Drück auf Starten', text: 'und der Hauptstrang geht sie für dich durch: er sagt jeder Karte, was die vorigen herausgefunden haben, entscheidet, ob sie fertig ist, und beantwortet die Rückfragen, bei denen sie stehen bleiben.' },
      kept: { lead: 'Im Repository abgelegt', text: 'reist ein Szenario mit und jeder, der dort arbeitet, hat es; in jedem Projekt abgelegt gehört es nur dir und begleitet dich.' },
      watch: { lead: 'Ein Lauf öffnet sich in einem eigenen Tab', text: 'mit jedem Schritt der Reihe nach. Pausieren, weitermachen, stoppen - und ein Druck auf einen Schritt zeigt sein ganzes Gespräch.' },
    },
    log: {
      loading: 'Lese, was sie gesagt hat…',
      step: 'Schritt',
      head: 'Hauptstrang',
    ...
    editor: {
      title: 'SZENARIO',
      newTitle: 'NEUES SZENARIO',
      save: 'Speichern',
      problems: 'Starten geht erst, wenn das behoben ist:',
      warnings: 'WÄRE AUFZURÄUMEN',
      issues: (n: number): string => (n === 1 ? '1 Problem' : `${n} Probleme`),
      outline: 'DAS SZENARIO',
      nameAndShelf: 'Name und Regal',
      name: 'NAME',
      shelf: 'LIEGT IN',
      inRepository: 'Diesem Repository',
      mine: 'Jedem Projekt',
      head: 'Hauptstrang',
      headNote: 'liest das und beurteilt danach jede Karte',
      briefingHint: 'Wofür dieser Arbeitsablauf da ist, in deinen Worten. Der Hauptstrang liest das und beurteilt danach jede Karte.',
      defaultModel: 'Wie ein neuer Tab',
      defaultEffort: 'Wie ein neuer Tab',
      sameAsHead: 'Wie der Hauptstrang',
      ...
      inputs: 'Fragt nach',
      inputsLabel: 'WIRD VOR DEM START GEFRAGT',
      addInput: 'Eine Frage hinzufügen',
      noInputs: 'Dieses Szenario fragt vor dem Start nichts.',
      inputName: 'Name',
      inputLabel: 'Wie es im Formular heißen soll',
      required: 'muss ausgefüllt sein',
      stages: 'STUFEN',
      addStage: 'Eine Stufe hinzufügen',
      stageNumber: (n) => `Stufe ${n}`,
      stageHead: (n: number, title: string): string => `STUFE ${n} · ${title}`,
      passes: 'DURCHGÄNGE',
      once: 'einer',
      times: (n) => `${n}`,
      goesRound: (n: number): string => `läuft ×${n} durch`,
      goesRoundUntil: (n: number): string => `läuft ×${n} durch, bis es fertig ist`,
      roundsShort: (n: number): string => `×${n}`,
      roundsUntilShort: (n: number): string => `×${n}, bis fertig`,
      untilDone: 'oder weniger, wenn der Hauptstrang sagt, es ist fertig',
      cards: 'KARTEN',
      addCard: 'Eine Karte hinzufügen',
      addCardHere: 'Eine Karte zu dieser Stufe hinzufügen',
      cardTitle: 'Wie diese Karte heißt',
      prompt: 'WAS SEINER SITZUNG GESAGT WIRD',
      promptHint: 'Was der eigenen Sitzung dieser Karte gesagt wird. {{Name}} wird vor dem Lauf gefragt; [[Name]] füllt der Hauptstrang aus dem, was die Karten darüber herausgefunden haben.',
      promptNote: 'Feldnamen sind hervorgehoben. Das ist das einzige Feld in der Konsolenschrift.',
      slots: 'FELDER, DIE ER FÜLLT',
      noSlots: 'Keine. Schreib [[Name]] in den Text und beschreibe es hier.',
      slotName: 'Name',
      slotHint: 'Was der Hauptstrang hier einsetzen soll',
      addSlot: 'Ein Feld hinzufügen',
      dod: 'FERTIG, WENN',
      dodHint: 'Woran der Hauptstrang erkennt, dass diese Karte fertig ist. Leer heißt: es reicht, dass der Zug endet.',
      after: 'WEITERGEBEN',
      afterHint: 'Was der Hauptstrang tun soll, wenn die Karte fertig ist: etwas prüfen, etwas notieren.',
      overrides: 'LÄUFT AUF',
      moveUp: 'Nach oben',
      moveDown: 'Nach unten',
      remove: 'Entfernen',
      untitledCard: 'Eine Karte ohne Namen',
      untitledStage: 'Eine Stufe ohne Namen',
    },
    problems: {
      noStages: 'Es enthält keine einzige Stufe.',
      emptyStage: 'Eine Stufe enthält keine einzige Karte.',
      noPrompt: (card) => `${card}: es steht nichts da, was ihre Sitzung tun soll.`,
      unknownInput: (card, name) => `${card}: {{${name}}} fragt dieses Szenario gar nicht ab.`,
      undeclaredSlot: (card, name) => `${card}: [[${name}]] steht im Text, aber niemand wird gebeten, es zu füllen.`,
      unusedSlot: (card, name) => `${card}: das Feld ${name} ist beschrieben, kommt im Text aber nicht vor.`,
      duplicateInput: (name) => `Zwei Fragen heißen beide ${name}.`,
      duplicateSlot: (card, name) => `${card}: zwei Felder heißen beide ${name}.`,
      badInputName: (name) => `„${name}“ ist kein Name - erlaubt sind Buchstaben, Ziffern, - und _.`,
      badSlotName: (card, name) => `${card}: „${name}“ ist kein Name - erlaubt sind Buchstaben, Ziffern, - und _.`,
    },
  },
  ...
  models: {
    ...
    fable: { sub: 'Fable 5.1 · Für die schwersten und längsten Aufgaben' },
    ...
    addSub: 'Eines, das Claude Code nicht anbietet',
  },
  ...
  header: {
    ...
    scenarios: 'Szenarien',
    closeScenarios: 'Szenarien schließen',
  ...
  mobile: {
    ...
    scenarios: {
      ...
      row: {
        runNow: 'Jetzt starten',
        asksFirst: (names: string): string => `fragt nach ${names}`,
        schedule: 'Lauf einplanen',
        scheduled: (n: number): string => `${n} gesetzt`,
        editor: 'Editor öffnen',
        duplicate: 'Duplizieren',
        delete: 'Dieses Szenario löschen',
      },
      start: {
        title: 'Jetzt starten',
      ...
      editor: {
        tidy: (n: number): string => (n === 1 ? '1 wäre aufzuräumen' : `${n} wären aufzuräumen`),
        tooBig: 'Dieses Szenario ist zu groß, um es von hier zu öffnen. Am Schreibtisch lässt es sich bearbeiten.',
        card: (stage: number, at: number, total: number): string => `Stufe ${stage} · Karte ${at} von ${total}`,
        done: 'Fertig',
      },
      step: {
        told: 'WAS IHM GESAGT WURDE',
        verdict: 'URTEIL',
        of: (at: number, took: string): string => `Schritt ${at} · ${took}`,
      },
    },
