import type { DayHours } from '../../stats/compute'
        ...
        <div className={s.dayBars}>
          {hours.bars.map((bar) => (
            <span
              key={bar.hour}
              className={s.dayBar}
              data-tooltip={`${clock(bar.hour)} · ${bar.minutes > 0 ? duration(bar.minutes) : 'quiet'}`}
            >
              <span
                className={`${s.dayBarFill} ${bar.minutes === 0 ? s.dayBarEmpty : ''}`}
                data-peak={hours.peak !== null && bar.hour === hours.peak.hour ? '' : undefined}
